    ...
    <div className="max-w-2xl mx-auto space-y-8 p-6">
      ...
      {user ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex items-center justify-between">
            ...
            <button onClick={() => setShowAuthModal(true)} className="px-6 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-lg font-bold text-sm flex items-center gap-2">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
                ...
                <button onClick={handleToggleSync} className={`px-6 py-3 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${syncOn ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-slate-700 hover:bg-slate-600 text-white'}`}>
                    {syncOn ? (<><CheckCircle size={16} /> Auto-Sync Active</>) : ('Enable Auto-Sync')}
                </button>
                {syncOn && (
        ...
        <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
                ...
                <div className="flex gap-2 mb-2">
                    <input type="text" placeholder="http://192.168.1.50:8080/scan-drop" value={scannerUrl} onChange={(e) => setScannerUrl(e.target.value)} className="flex-1 bg-slate-950 border border-slate-700 rounded-lg px-4 py-2 text-white text-sm focus:border-purple-500 outline-none" />
                    <button onClick={handleSaveScanner} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg font-medium text-sm">
                        Connect
                    </button>
                </div>
                {savedScannerUrl && (
