      ...
      if(data.user) { 
        ...
        for (const asset of local) {
          if (asset.status === AssetStatus.MINTED && !asset.sqlRecord?.user_id) {
            try {
              // Sync locally processed assets to cloud (successful processing, no failure)
  ...
  const handleAssetUpdate = async (updatedAsset: DigitalAsset) => {
    if (!isGlobalView) {
        ...
        if (user?.id) {
          ...
          type LicenseType = 'GEOGRAPH_CORPUS_1.0' | 'CC0';
          const license: LicenseType = isPublicBroadcast ? 'CC0' : 'GEOGRAPH_CORPUS_1.0';
