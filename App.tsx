...
import { AssetStatus, DigitalAsset, HistoricalDocumentMetadata, BatchItem, ImageBundle, ScanType, SCAN_TYPE_CONFIG, GraphData, GraphNode } from './types';
import { processImageWithGemini } from './services/geminiService';
import { createBundles } from './services/bundleService';
import { initSync } from './lib/syncEngine';
import { loadAssets, saveAsset } from './lib/indexeddb';
...
export default function App() {
  ...
  const processAssetPipeline = async (asset: DigitalAsset, file: File) => {
      ...
      contributeAssetToGlobalCorpus(resultAsset, user?.id).then(syncResult => {
        ...
        <div className="flex-1 overflow-auto p-8 relative">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-8 max-w-6xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <StatCard label="Active Bundles" value={displayItems.filter(i => 'bundleId' in i).length} icon={Package} color="text-amber-500" onClick={() => setActiveTab('assets')} />
              </div>
              ...
              {assets.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    <h3 className="text-white font-medium mb-4 flex items-center gap-2"><Globe size={18} className="text-emerald-500"/> GIS Context</h3>
                        ...
                        {assets.slice(0, 3).map(asset => (
                            <div key={asset.id} className="flex items-start gap-4 p-3 rounded bg-slate-950/50 border border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 mt-1 line-clamp-2">{asset.processingAnalysis || asset.sqlRecord?.DOCUMENT_DESCRIPTION}</p>
                                </div>
                            </div>
                        ))}
                    </div>
          ...
          {activeTab === 'database' && (
             <div className="h-full flex flex-col gap-4">
               ...
               {dbViewMode === 'DRILLDOWN' && (
                 <div className="flex-1 overflow-auto bg-slate-900 border border-slate-800 rounded-xl shadow-inner relative">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-950 sticky top-0 z-10">
                          <tr>
                            {['ID', 'TITLE', 'COLLECTION', 'GIS ZONE', 'NODES', 'CATEGORY', 'DESCRIPTION', 'ACTION'].map(h => (
          ...
          {activeTab === 'assets' && (
             <div className="h-full overflow-y-auto">
                 <div className="flex justify-between items-center mb-6">
                     <h3 className="text-lg font-bold text-white">Analyzed Assets & Bundles</h3>
                 </div>
          ...
          {activeTab === 'graph' && (
            <div className="flex gap-6 h-full flex-col">
               <div className="flex items-center justify-between"><h3 className="text-lg font-bold text-white">Global Knowledge Graph</h3></div>
               <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 p-4 flex flex-col relative overflow-hidden">
                    <GraphVisualizer data={globalGraphData} width={1000} height={600} />
               </div>
            </div>
          )}

          {activeTab === 'settings' && <SettingsPanel onOpenPrivacy={() => {}} />}
